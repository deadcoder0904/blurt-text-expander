...
function getBuildEnv() {
  ...
  const ENTRIES = [
    ...
    path.resolve('src/options/options.ts'),
    path.resolve('src/content/content.ts'),
  ]
...
async function relocateCompiledOutputs(dist: string) {
  ...
  // We want: background.js, popup.js, options.js, content.js at dist root
  const pairs: Array<[string, string]> = [
    ...
    [path.join(dist, 'options', 'options.js'), path.join(dist, 'options.js')],
    [path.join(dist, 'content', 'content.js'), path.join(dist, 'content.js')],
  ]
...
async function css(env: ReturnType<typeof getBuildEnv>) {
  const pairs = [
    ...
    ['src/styles/options.css', path.join(env.DIST, 'styles/options.css')],
  ] as const
...
async function html(env: ReturnType<typeof getBuildEnv>) {
  ...
  await cp('src/options/options.html', path.join(env.DIST, 'options.html'))
}
